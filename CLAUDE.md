...
- **Set-up:** the laptop (Windows, `desktop-tmsridg`) runs Cowork or the Code tab and reaches GitHub over SSH; its sandbox **cannot reach the home LAN**. Garcks-PC (Linux Mint, 192.168.1.157) runs Planet as `planet.service` on port 8080 and runs the front door; Claude Code there is "PC Claude" — Jamie calls it **Desktop Claude** or **Linux Claude**, the one given runbook commands. **Planet Claude** is a different Claude, in Planet's own root folder, for long many-seed runs and core Planet work; it takes no Workshop steps. Viewer graphics are done in a laptop Planet session (Jamie, W4). Home connection 75 Mbps down, 17.5 Mbps up. **Performance here means the upload and the cache**, not frame time: the budget is in the Strategic Plan.
...
16. **Two Claudes and a bot, one branch (Jamie, 24 Sep 2026; the bot W3-e).** Laptop Claude owns the repo and makes every commit, with two exceptions: PC Claude commits only under `frontdoor/state/` — what it installed on Garcks-PC, from which commit, when, and what `nginx -t` and the service status said; and the still's GitHub Action (`.github/workflows/still.yml`) commits only under `site/public/still/`, refusing anything else, so laptop sessions `pull` its daily commits. Both `git pull` before a session and `git push` at its end. Reports between the two go through `frontdoor/state/`, not the Desktop note, once the repo exists. PC Claude never edits the quartet, the site or the configs; a change it needs is written into Active-Work's queue.
