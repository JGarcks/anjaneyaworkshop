...
| The same public key on Garcks-PC | `~/.ssh/authorized_keys` on Garcks-PC | Unused: the laptop's sandbox cannot reach the LAN. Harmless; remove if wanted. | 24 Sep 2026 |
| Cloudflare account | Jamie's Cloudflare account. No API token exists: Pages builds by watching the repo through Cloudflare's GitHub app (W1-a) | Jamie | 24 Sep 2026 (W1) |
