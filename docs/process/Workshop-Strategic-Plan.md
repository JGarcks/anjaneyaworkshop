...
- **The door's replies:** gzip on for JSON and binary fields; `Cache-Control: public, max-age=0, s-maxage=1` on `/api/meta`, `/api/picture/*`, `/api/field/*`, `/api/events`, `/api/log` (one second at the edge, none in the browser); a day on `/api/grid`; `X-Planet-Tick` present on fields and pictures; the door holds the fields a second (keeping them in step, W4-e) but passes the picture and meta straight through (W5-b, W5-c; confirmed W8-c); POST → 405; the second fetch of the same field within a second a cache hit at the edge.
- **The engine's load:** about one request per URL per second however many browsers watch, read at nginx (`frontdoor/door-rate.sh`) — except the picture and meta, which the door does not hold: one each a second per Cloudflare location with a viewer (W5-b, W5-c; confirmed W8-c). *(W4-e briefly did the same for every live number; undone the same evening, since the separate fields fell out of step.)*
