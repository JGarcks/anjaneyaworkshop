...
| 2 The landing page | open | 24 Sep 2026 (W3) | | Live at the public address; 51 tests; check-public 17/17 (Phase 2 assertions come in W4). Still at 0.7 s, live at ~3.5 s (a fixed 2.5 s wait until Planet's `?embed` "drawn" message). Words at the very bottom (Jamie's first look). Walk: works on laptop and phone (after the zone's Browser Cache TTL fix, W3-i, and `style.css?v=w3`) |
