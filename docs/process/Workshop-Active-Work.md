...
**Next build session: W9 — the rented server (Phase 4 brought forward, Jamie 26 Sep: WS · D7 move the engine, W9-a…c).** Decision round done 26 Sep (`decisions.md` WS · D2, D7, D13, W9-a…c). OVHcloud VPS-1, Erith, Ubuntu 26.04, order 259163206 placed 26 Sep, payment validating; waits for its IP. Found in prep: the engine listens on 0.0.0.0:8080 with no bind flag (the server needs a firewall and the service locked to loopback; a bind flag is a Planet request); the live planet moved to `planet-d16` from year zero at ~17:20 on 26 Sep (Planet's SC · D16), so which world goes up is re-put to Jamie; the tunnel's credentials and stopping the PC's tunnel need sudo on Garcks-PC (Desktop Claude or Jamie).

**Then W10 — the Planet half of the performance round, then the gate (W8, the hub's half, done; split W8-d). Waits for item 16 live and measured.** At its decision round, W6's recommendations as revised in W7:
