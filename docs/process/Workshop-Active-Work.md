...
0. **Planet item 7 in flight (W5-a, Jamie: now, one request per picture).** Handed over at the end of `Desktop/for-laptop-claude.txt`: Desktop Claude installs the door's new address first (runbook §For Planet item 7), then Planet Claude builds `/api/picture/<field>` (contract: `requests/planet-embed.md` §Item 7) and goes live at Jamie's word. Wait for both replies there. Then: `scripts/hub-monitor.mjs` 5 min (freezes gone?), `check-public.sh` gains the picture address, the door's hold tried off again (W4-e), measured.
1. **The glide's feel (Jamie, end of W4): "too drawn out — too slow and still not keeping up with itself"** after Planet's `?embed` glide change (2.5 pictures behind, 5 s). Retune only after item 7 and its measure: `scripts/hub-monitor.mjs` (before item 7: median 2.1 s apart, freezes of 10–38 s) plus the viewer's own timing readout (share of frames at rest). Any fix is Planet's (`docs/process/requests/planet-embed.md`).
...
**Pending Jamie:** the word for item 7's two steps (the door's `sudo` line; Planet's restart) · swipe-to-refresh: leave it, or try the top strip catching the pull (real phone only) · WS · D5, BlockByBlock's new name (Phase 3).
