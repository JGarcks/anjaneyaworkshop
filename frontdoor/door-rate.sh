...
# Built in W2 — the front door (24 Sep 2026). PC Claude runs it at the Phase 1 gate while three browsers watch; the numbers go into state/.
set -euo pipefail
...
# An engine fetch is a MISS or an EXPIRED; a HIT or an UPDATING was answered from the door's memory.
awk -v since="$SINCE" -v window="$WINDOW" '
  $1 >= since { n++; bytes += $6; if ($5 == "MISS" || $5 == "EXPIRED") { up[$3]++; fetches++ } }
  END {
