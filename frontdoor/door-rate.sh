...
# Decision: the budget is measured at nginx (Strategic Plan §Budget: under 0.6 MB/s; about one engine fetch per address per second).
# Built in W2 — the front door (24 Sep 2026); W4 counts BYPASS too, in case a path is ever passed straight through. PC Claude runs it at the Phase 1 gate while three browsers watch; the numbers go into state/.
set -euo pipefail
...
# An engine fetch is a MISS, an EXPIRED or a BYPASS (a path passed through, as W4-e briefly did); a HIT or an UPDATING
